...
const dataConfigs = [
  { dataKey: 'co2', unit: 'ppm' },
  { dataKey: 'temperature', unit: '°C' },
  { dataKey: 'humidity', unit: '%' },
  { dataKey: 'pressure', unit: 'hPa' },
...
export const Aranet4Charts = () => {
  ...
  const fetchData = useCallback(async () => {
    const response = await fetch('/api/experiments?timeseries=aranet');
    if (!response.ok) {
      return;
    }
