...
export const loader = async () => {
  const [aranetAveragesLast24h, lastAranetReading] = await Promise.all([
    AranetReadings.getLast24h(),
    AranetReadings.getLast(),
  ]);

  return { aranetAveragesLast24h, lastAranetReading };
...
export default function Experiments() {
  ...
  const lastAranetReadingUpdate = useTimeAgo(lastAranetReading.createdAt);
    ...
    <>
      ...
      {!!lastAranetReading && (
        <p className="flex flex-col sm:flex-row gap-1 justify-start sm:justify-between mt-2">
          <span>last updated: {lastAranetReadingUpdate}</span>
